...
面向手机的中文运动计划与打卡 Web App。当前版本包括邮箱注册/登录、资料初始化、自定义周期计划、未来 90 天日程、一键打卡、撤销、日历、统计、公开打卡主页、私密体重/BMI、归一化公开体重趋势、关注动态、点赞、每日催促、通知中心、个人设置和可安装 PWA。Web 端只负责计划和记录，不包含“打开视频”或外部视频链接。
...
数据库命令必须在已链接的测试项目上运行。当前准确状态见 [核心验证记录](docs/verification/core.md)，RLS 设计见 [安全说明](docs/security/rls.md)。
...
当前仍未完成打卡照片和 Web Push 发送服务；不得用空页面或虚假数据冒充这些能力。真实体重仅本人可访问，公开页面只读取数据库生成的归一化趋势。好友动态仅返回安全聚合字段。
