  ...
  return <main>
    ...
    <section className="space-y-3">
      ...
      {day.occurrences.map((occurrence) => {
        ...
        return <article key={occurrence.id} className="rounded-2xl bg-white p-5">
          ...
          <div className="mt-4 flex items-center gap-3"><Link href={`/plans/${occurrence.plan_id}`} className="text-sm font-semibold text-slate-600">编辑计划</Link>{checkin ? <span className="ml-auto text-sm font-bold text-emerald-600">✓ 已完成</span> : canComplete ? <form action={completeOccurrenceAction} className="ml-auto"><input type="hidden" name="occurrenceId" value={occurrence.id}/><input type="hidden" name="checkinDate" value={date}/><button className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-bold text-white">完成这项计划</button></form> : <span className="ml-auto text-sm text-slate-400">{pendingLabel}</span>}</div>
        </article>;
      })}
    </section>

    {day.canBackfill && <form action={manualCheckinAction} className="mt-4"><input type="hidden" name="checkinDate" value={date}/><button className="w-full rounded-2xl border border-sky-200 bg-white px-4 py-4 font-semibold text-sky-700">{date === day.today ? "临时打卡" : "补录当天运动"}</button></form>}
    ...
    <section className="space-y-3">
      ...
      {day.checkins.map((checkin) => <article key={checkin.id} className="rounded-2xl bg-white p-5">
        ...
        <details className="mt-4"><summary className="cursor-pointer text-sm font-semibold text-emerald-700">补充记录</summary><form action={updateCheckinDetailsAction} className="mt-3 space-y-3"><input type="hidden" name="checkinId" value={checkin.id}/><input type="hidden" name="checkinDate" value={date}/><label className="block text-sm">实际分钟（可选）<input name="durationMinutes" type="number" min={1} max={1440} defaultValue={checkin.duration_minutes ?? ""} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2"/></label><label className="block text-sm">运动内容（可选）<input name="activityText" maxLength={500} defaultValue={checkin.activity_text ?? ""} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2"/></label><label className="block text-sm">备注（可选）<textarea name="notes" maxLength={2000} defaultValue={checkin.notes ?? ""} className="mt-1 min-h-20 w-full rounded-xl border border-slate-200 px-3 py-2"/></label><button className="w-full rounded-xl bg-slate-900 px-4 py-3 font-bold text-white">保存补充记录</button></form></details>
        <form action={undoCheckinAction} className="mt-3"><input type="hidden" name="checkinId" value={checkin.id}/><input type="hidden" name="checkinDate" value={date}/><button className="min-h-11 text-sm font-semibold text-rose-600">撤销这次打卡</button></form>
      </article>)}
    </section>
  </main>;
}
