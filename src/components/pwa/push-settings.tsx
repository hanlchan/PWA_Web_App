...
export function PushSettings() {
  ...
  return <section className="mt-6 rounded-2xl bg-white p-4"><h2 className="font-bold">系统运动提醒</h2><p className="mt-1 text-xs leading-5 text-slate-500">只有点击下方按钮后才会申请通知权限。更换网站域名后，原域名的推送订阅不能复用，请在当前域名重新开启。拒绝或不支持通知不会影响计划与打卡。</p>{ios && !standalone && <p className="mt-2 rounded-xl bg-amber-50 p-3 text-xs text-amber-800">iPhone/iPad：请先将网站添加到主屏幕，再开启通知。</p>}<div className="mt-4 flex gap-2"><button type="button" onClick={enable} disabled={pending} className="min-h-11 flex-1 rounded-xl bg-emerald-500 px-3 text-sm font-bold text-white disabled:opacity-50">{pending ? "处理中…" : "开启运动提醒"}</button><button type="button" onClick={disable} disabled={pending || !supported} className="min-h-11 rounded-xl border border-slate-200 px-3 text-sm font-semibold disabled:opacity-50">{pending ? "处理中…" : "关闭"}</button></div>{message && <p role="status" className="mt-3 text-sm text-slate-600">{message}</p>}</section>;
}
