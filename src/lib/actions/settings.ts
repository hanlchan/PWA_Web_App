...
export async function updateSettingsAction(form: FormData) {
  ...
  if (!parsed.success) throw new Error(parsed.error.issues[0]?.message ?? "资料无效");
  ...
  if (error) throw new Error("资料更新失败");
